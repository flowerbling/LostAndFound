  ...
  methods: {
    ...
    getAllGoods() {
      ...
      if (this.the_type === 'all') {
        this.$axios.get(this.$settings.HOST + 'goods/pick_info/', {params: filters}).then(
          res => {
        ...
        this.$axios.get(this.$settings.HOST + 'goods/pick_info/?the_type=' + this.the_type, {params: filters}).then(
          res => {
            this.the_list = res.data.results
            this.total = res.data.count
